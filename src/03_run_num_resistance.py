...
cL_range = np.logspace(0,4,10 * 4 + 1,endpoint = True)
...
if select_heads:
    ...
    for ic, c_L in enumerate(cL_range):
        ...
        Ex.read_num_from_pickle(
            file_results = file_results.format(abs(np.log10(c_L)))) 
...
if inverse_est:   
    print('### Inverse estimation for resistance values ###\n')
    ...
    print('### Inverse estimation finished ###\n')
    ...
    plt.title("Diffusivity fit at $x = {:.0f}$m as function of resistance value".format(x_piez))
    plt.xlabel("Resistance $c$ [d]")
